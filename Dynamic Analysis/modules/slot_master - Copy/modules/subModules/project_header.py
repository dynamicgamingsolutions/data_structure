...
def project_header(file_name, sheet, conn):
    ...
    if len(header_row_index) > 1:
        ...
        df, df_old = conform_dataframe(conn, file_name, sheet, header_row_index)
    else: 
        #Assign index to header_row_index if it is not empty
        df = pd.read_excel(file_name, sheet_name=sheet, skiprows=header_row_index[0] if header_row_index else 0, header=0, dtype=str)
